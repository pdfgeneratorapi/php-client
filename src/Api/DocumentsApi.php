...
class DocumentsApi
{
     ...
     * @param  string|null $name Document name, returned in the meta data. (optional)
     * @param  string|null $format Document format. The zip option will return a ZIP file with PDF files. (optional)
     * @param  string|null $output Response format. \&quot;I\&quot; is used to return the file inline. With the url option, the document is stored for 30 days and automatically deleted. (optional)
    ...
    public function mergeTemplatesRequest($batch_data_inner, $name = null, $format = null, $output = null, string $contentType = self::contentTypes['mergeTemplates'][0])
    {
        ...
        if (isset($batch_data_inner)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = \GuzzleHttp\Utils::jsonEncode(ObjectSerializer::sanitizeForSerialization($batch_data_inner));
            } else {

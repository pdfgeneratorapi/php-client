...
class AssetsApi
{
     ...
     * Generate QR Code
     *
     * @param  \PDFGeneratorAPI\Model\GenerateQRCodeRequest $generate_qr_code_request QR Code configuration (required)
     ...
     * @return \PDFGeneratorAPI\Model\GenerateQRCode201Response|\PDFGeneratorAPI\Model\InlineObject21|\PDFGeneratorAPI\Model\InlineObject22|\PDFGeneratorAPI\Model\InlineObject23|\PDFGeneratorAPI\Model\InlineObject24|\PDFGeneratorAPI\Model\InlineObject25|\PDFGeneratorAPI\Model\InlineObject26|\PDFGeneratorAPI\Model\InlineObject27
     */
    public function generateQRCode($generate_qr_code_request, string $contentType = self::contentTypes['generateQRCode'][0])
    {
        list($response) = $this->generateQRCodeWithHttpInfo($generate_qr_code_request, $contentType);
        return $response;
    }
     ...
     * @throws \PDFGeneratorAPI\ApiException on non-2xx response or if the response body is not in the expected format
     * @throws \InvalidArgumentException
     * @return array of \PDFGeneratorAPI\Model\GenerateQRCode201Response|\PDFGeneratorAPI\Model\InlineObject21|\PDFGeneratorAPI\Model\InlineObject22|\PDFGeneratorAPI\Model\InlineObject23|\PDFGeneratorAPI\Model\InlineObject24|\PDFGeneratorAPI\Model\InlineObject25|\PDFGeneratorAPI\Model\InlineObject26|\PDFGeneratorAPI\Model\InlineObject27, HTTP status code, HTTP response headers (array of strings)
     */
    public function generateQRCodeWithHttpInfo($generate_qr_code_request, string $contentType = self::contentTypes['generateQRCode'][0])
    {
        $request = $this->generateQRCodeRequest($generate_qr_code_request, $contentType);
            ...
            switch($statusCode) {
                case 201:
                    return $this->handleResponseWithDataType(
                        '\PDFGeneratorAPI\Model\GenerateQRCode201Response',
                        $request,
                        $response,
                    );
                case 401:
     ...
     */
    public function generateQRCodeAsync($generate_qr_code_request, string $contentType = self::contentTypes['generateQRCode'][0])
    {
        return $this->generateQRCodeAsyncWithHttpInfo($generate_qr_code_request, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function generateQRCodeAsyncWithHttpInfo($generate_qr_code_request, string $contentType = self::contentTypes['generateQRCode'][0])
    {
        $returnType = '\PDFGeneratorAPI\Model\GenerateQRCode201Response';
        $request = $this->generateQRCodeRequest($generate_qr_code_request, $contentType);
     ...
     */
    public function generateQRCodeRequest($generate_qr_code_request, string $contentType = self::contentTypes['generateQRCode'][0])
    {

        // verify the required parameter 'generate_qr_code_request' is set
        if ($generate_qr_code_request === null || (is_array($generate_qr_code_request) && count($generate_qr_code_request) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $generate_qr_code_request when calling generateQRCode'
            );
        }


        $resourcePath = '/assets/qrcode';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
        ...
        // for model (json/xml)
        if (isset($generate_qr_code_request)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = \GuzzleHttp\Utils::jsonEncode(ObjectSerializer::sanitizeForSerialization($generate_qr_code_request));
            } else {
                $httpBody = $generate_qr_code_request;
            }
